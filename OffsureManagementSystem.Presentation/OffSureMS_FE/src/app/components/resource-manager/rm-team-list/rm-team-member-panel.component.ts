...
@Component({
  selector: 'app-rm-team-member-panel',
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, GenericFormComponent, TeamMemberSkillsEditorComponent],
  templateUrl: './rm-team-member-panel.component.html',
  styleUrl: './rm-team-member-panel.component.scss',
})
export class RmTeamMemberPanelComponent implements OnChanges, OnDestroy {
  ...
  saveEdit(): void {
    ...
    this.saving = true;
    this.portal
      .updateTeamMember(this.member.id, {
        ...
        yearsOfExperience:
          raw.yearsOfExperience != null && raw.yearsOfExperience !== '' ? Number(raw.yearsOfExperience) : 0,
  ...
  async deactivateMember(): Promise<void> {
    ...
    this.portal.deactivateTeamMember(this.member.id).subscribe({
      next: res => {
        this.member = res.data ?? this.member;
        this.patchForm(this.member!);
        this.editing = false;
  ...
  async activateMember(): Promise<void> {
    ...
    this.portal.activateTeamMember(this.member.id).subscribe({
      next: res => {
        this.member = res.data ?? this.member;
        this.patchForm(this.member!);
  ...
  async deleteMember(): Promise<void> {
    ...
    this.deleting = true;
    this.portal.deleteTeamMember(this.member.id).subscribe({
      next: () => {
  ...
  private loadMember(): void {
    ...
    this.portal
      .getTeamMemberById(this.memberId)
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: res => {
